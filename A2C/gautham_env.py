...
class SumoEnvironment(gym.Env):
    ...
    def get_run(self):
        while True:
            if os.path.exists(f"./A2C/runs/{self.network}/run_{self.run}"):
                self.run += 1
            else:
                break
        
        if not os.path.exists(f"./A2C/runs/{self.network}"):
            os.mkdir(f"./A2C/runs/{self.network}")
        os.mkdir(f"./A2C/runs/{self.network}/run_{self.run}")
        # os.mkdir(f"./A2C/runs/{self.network}/run_{self.run}/logs")
        # with open(f"./A2C/runs/{self.network}/run_{self.run}/logs.csv", "a") as f:
        #     print("Episode,")
    ...
    def _start_evaluation(self):
        ...
        # f"./A2C/esults/{self.rPath}/Summary.xml",
        # "--queue-output",
        # f"./results/{self.rPath}/QueueInfo.xml",
        # "--tripinfo-output",
        # f"./results/{self.rPath}/VehicleInfo.xml"])
        if not os.path.exists(f"./A2C/runs/{self.network}/run_{self.run}/Summary_Eval"):
            os.mkdir(f"./A2C/runs/{self.network}/run_{self.run}/Summary_Eval")
        if not os.path.exists(f"./A2C/runs/{self.network}/run_{self.run}/QueueInfo_Eval"):
            os.mkdir(f"./A2C/runs/{self.network}/run_{self.run}/QueueInfo_Eval")
        if not os.path.exists(f"./A2C/runs/{self.network}/run_{self.run}/VehicleInfo_Eval"):
            os.mkdir(f"./A2C/runs/{self.network}/run_{self.run}/VehicleInfo_Eval")

        sumo_cmd.extend(["--summary",
                         f"./A2C/runs/{self.network}/run_{self.run}/Summary_Eval/episode_{self.episode}.xml",
                         "--queue-output",
                         f"./A2C/runs/{self.network}/run_{self.run}/QueueInfo_Eval/episode_{self.episode}.xml",
                         "--tripinfo-output",
                         f"./A2C/runs/{self.network}/run_{self.run}/VehicleInfo_Eval/episode_{self.episode}.xml"])
    ...
    def _start_simulation(self):
        ...
        # "--tripinfo-output",
        # f"./results/{self.rPath}/VehicleInfo.xml"])
        if not os.path.exists(f"./A2C/runs/{self.network}/run_{self.run}/Summary"):
            os.mkdir(f"./A2C/runs/{self.network}/run_{self.run}/Summary")
        if not os.path.exists(f"./A2C/runs/{self.network}/run_{self.run}/QueueInfo"):
            os.mkdir(f"./A2C/runs/{self.network}/run_{self.run}/QueueInfo")
        if not os.path.exists(f"./A2C/runs/{self.network}/run_{self.run}/VehicleInfo"):
            os.mkdir(f"./A2C/runs/{self.network}/run_{self.run}/VehicleInfo")

        sumo_cmd.extend(["--summary",
                         f"./A2C/runs/{self.network}/run_{self.run}/Summary/episode_{self.episode}.xml",
                         "--queue-output",
                         f"./A2C/runs/{self.network}/run_{self.run}/QueueInfo/episode_{self.episode}.xml",
                         "--tripinfo-output",
                         f"./A2C/runs/{self.network}/run_{self.run}/VehicleInfo/episode_{self.episode}.xml"])
